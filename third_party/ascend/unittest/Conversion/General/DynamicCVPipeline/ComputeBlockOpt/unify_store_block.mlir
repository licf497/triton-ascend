...
module {
  ...
  // ============================================
  // Test 3: Block argument source - no producer, skip
  //
  // Pattern:
  //   materialize.source = %in (function argument, block argument)
  //   -> traceProducerOp hits block argument, returns nullptr -> skip.
  //
  // No producer to unify with, so all block_ids remain unchanged.
  // ============================================
  // CHECK-LABEL: func @test_block_arg_source_skip
  func.func @test_block_arg_source_skip(%arg0: memref<?xf16> {tt.divisibility = 16 : i32}, %in: tensor<64x64xf16>) {
    // CHECK: arith.constant {ssbuffer.block_id = 30 : i32, ssbuffer.core_type = "VECTOR"} 0 : index
    %c0 = arith.constant {ssbuffer.block_id = 30 : i32, ssbuffer.core_type = "VECTOR"} 0 : index
    // CHECK: memref.reinterpret_cast %{{.*}} {ssbuffer.block_id = 30 : i32, ssbuffer.core_type = "VECTOR"} : memref<?xf16> to memref<128x64xf16, strided<[64, 1], offset: ?>>
    %reinterpret = memref.reinterpret_cast %arg0 to offset: [%c0], sizes: [128, 64], strides: [64, 1] {ssbuffer.block_id = 30 : i32, ssbuffer.core_type = "VECTOR"} : memref<?xf16> to memref<128x64xf16, strided<[64, 1], offset: ?>>
    // CHECK: memref.subview %{{.*}} {ssbuffer.block_id = 30 : i32, ssbuffer.core_type = "VECTOR"} : memref<128x64xf16, strided<[64, 1], offset: ?>> to memref<64x64xf16, strided<[64, 1], offset: ?>>
    %subview = memref.subview %reinterpret[0, 0] [64, 64] [1, 1] {ssbuffer.block_id = 30 : i32, ssbuffer.core_type = "VECTOR"} : memref<128x64xf16, strided<[64, 1], offset: ?>> to memref<64x64xf16, strided<[64, 1], offset: ?>>
    // CHECK: bufferization.materialize_in_destination %{{.*}} {ssbuffer.block_id = 30 : i32, ssbuffer.core_type = "VECTOR"} : (tensor<64x64xf16>, memref<64x64xf16, strided<[64, 1], offset: ?>>) -> ()
    bufferization.materialize_in_destination %in in writable %subview {ssbuffer.block_id = 30 : i32, ssbuffer.core_type = "VECTOR"} : (tensor<64x64xf16>, memref<64x64xf16, strided<[64, 1], offset: ?>>) -> ()
    return
  }

  // ============================================
  // Test 5: Store inside scf.for with scalar index dependency
  //
  ...
  // ============================================
  // Test 6: hivm.hir.store use case
  //
  ...
  // ============================================
  // Test 7: Multiple vector ops in chain - producer is the first vector
